...
def main():
    ...
    mass_of_chemical_sample = float(input("Enter the mass in grams of the sample: "))
    ...
    number_of_moles = mass_of_chemical_sample / total_molar_mass    

    # Print the molar mass.

    print(f"{round(total_molar_mass, 5)} grams/mole ")
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for index in range(len(symbol_quantity_list)): 
         ...
         # Retrieve one inner list from the compound list  symbol_quantity_list.
        list_element = symbol_quantity_list[index]

        # Retrieve symbol from the inner list.
        symbol = list_element[SYMBOL_INDEX]

       # Retrieve quantity of atom from the inner list.
        quantity_of_atom = list_element[QUANTITY_INDEX]

        # Retrieve one value from the periodic_table_dict.
        dict_value = periodic_table_dict[symbol]

        # Retrieve symbol name from the inner list.
        atom_name = dict_value[NAME_INDEX]

        # Retrieve atomic mass from the inner list.
        atomic_mass = dict_value[ATOMIC_MASS_INDEX]

        total_molar_mass += atomic_mass * quantity_of_atom
